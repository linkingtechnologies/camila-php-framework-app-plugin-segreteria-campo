// app-pre-accreditations-summary.js
...
async function mount() {
  try {
    const { PreAccreditationsSummary } = await import(
      `./views/pre-accreditations-summary/index.js?v=${VERSION}`
    );
    render(
      await PreAccreditationsSummary({ client, html, render, root }),
      root
    );
  } catch (e) {
    showError("Errore avvio", e);
  }
}
